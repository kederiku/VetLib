 ...
 * appels HTTP du portail B2C (propriétaires d'animaux) vers le backend
 * FastAPI. Toute préoccupation transverse (base URL, cookies d'auth,
 * parsing de la réponse, rafraîchissement silencieux du token,
 * transformation des erreurs) se règle ici une seule fois, jamais dans
 * les composants.
 */
...
// 1. Importer getOwnerRefreshTokenUrl() depuis le code généré créerait un
//    cycle d'imports (le généré importe déjà ce mutator) ;
// 2. Le cookie vetolib_owner_refresh est émis avec
//    Path=/api/v1/owner/auth/refresh : le navigateur ne le joint QUE sur
//    cette URL exacte. La figer ici rend ce contrat visible et intouchable
//    par une régénération Orval.
// Le B2C ne rafraîchit QUE la session OWNER : le client généré contient
// aussi les hooks staff (/api/v1/auth/*), mais ce portail ne les utilise
// jamais ; une session staff éventuelle (même navigateur connecté au B2B)
// ne doit ni être touchée ni servir de session ici.
...
// /api/v1/owner/auth/me n'y figure PAS : son 401 doit déclencher le
// refresh (c'est même le cas le plus courant : retour sur le portail
// après > 15 min).
...
/**
 * Tente de rafraîchir la session via le cookie vetolib_owner_refresh.
...
const refreshSession = (): Promise<boolean> => {
  if (refreshPromise === null) {
    refreshPromise = fetch(`${BASE_URL}${REFRESH_URL}`, {
      method: "POST",
      // Indispensable : envoie le cookie vetolib_owner_refresh et accepte
      // les nouveaux cookies Set-Cookie de la réponse (rotation des tokens).
      credentials: "include",
    })
...
export const customFetch = async <T>(
  ...
  // le frontend (:3000) et l'API (:8000) sont des origines différentes.
  const doFetch = () =>
    fetch(`${BASE_URL}${url}`, {
      ...options,
      // Cookies JWT HttpOnly (vetolib_owner_access / vetolib_owner_refresh)
