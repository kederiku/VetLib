/**
 * Hook useCurrentUser : l'UNIQUE source de vérité sur la session owner.
 *
 * Pas de Context React ni de store global pour "le propriétaire connecté" :
 ...
 * /api/v1/owner/auth/me), donc la même donnée et le même état de
 * chargement. Se connecter = remplir cette entrée (setQueryData après le
 * login), se déconnecter = la supprimer (removeQueries après le logout).
 */
"use client";

import { useGetCurrentOwner } from "@/lib/api/generated/owner-auth/owner-auth";

/**
 * Retourne la session courante sous forme de query TanStack :
 * - data : le OwnerResponse si connecté ;
 * - isPending : vérification en cours (au premier montage) ;
 * - isError : pas de session valide (401 même après refresh silencieux).
 */
export function useCurrentUser() {
  return useGetCurrentOwner({
    query: {
      ...
      // veulent que le OwnerResponse. select extrait .data UNE fois ici,
      // au lieu de forcer chaque appelant à écrire owner?.data partout.
      select: (res) => res.data,
    },
  });
}
