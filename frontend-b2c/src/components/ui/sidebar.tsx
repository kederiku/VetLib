/**
 * Composant Sidebar (preset shadcn/ui, variante Base UI).
 *
 * La navigation latérale complète du portail : un SidebarProvider
 * (contexte + état ouvert/replié persisté en COOKIE, pour survivre aux
 * rechargements), la Sidebar elle-même (en Sheet coulissant sur mobile,
 * détecté via useIsMobile), et des sous-composants de composition
 * (Header/Content/Footer, menus, bouton SidebarTrigger, SidebarInset
 * pour la zone de contenu). Composant COPIÉ dans le projet (philosophie
 * shadcn) : librement modifiable, pas de dépendance externe à suivre.
 *
 * DEUX RETOUCHES VOLONTAIRES du preset, à préserver si la CLI shadcn
 * réécrit un jour ce fichier :
 * 1. les libellés d'accessibilité (sr-only, aria-label, title) sont
 *    TRADUITS : un lecteur d'écran francophone ne doit pas annoncer
 *    "Toggle Sidebar" sur un portail grand public français ;
 * 2. data-active:text-sidebar-primary est ajouté aux classes du
 *    SidebarMenuButton, pour que l'entrée active prenne la teinte de
 *    marque (émeraude) et non le seul fond gris du preset.
 */
"use client"
      ...
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        ...
        >
          <SheetHeader className="sr-only">
            ...
            <SheetDescription>Menu de navigation du portail.</SheetDescription>
          </SheetHeader>
