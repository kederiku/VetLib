/**
 * Formulaire de connexion au portail propriétaires.
 *
 * Assemblage type de tous les formulaires du portail :
 ...
 * - le hook Orval useOwnerLogin exécute POST /api/v1/owner/auth/login ;
 * - applyServerErrors replace les erreurs API sous les bons champs.
 * Au succès, le backend a posé les cookies HttpOnly vetolib_owner_* :
 * le navigateur détient la session, le JS n'a jamais vu les tokens.
 */
"use client";
...
export function LoginForm() {
  ...
  const onSubmit = handleSubmit(async (values) => {
    try {
      ...
      // Le login renvoie déjà le OwnerResponse : on le range directement
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Retrouvez vos rendez-vous et les informations de vos animaux.
        </CardDescription>
      </CardHeader>
      <CardContent>
        ...
        <form onSubmit={onSubmit} noValidate>
          <FieldGroup>
            ...
            {/* Passerelle vers l'inscription. Base UI n'a pas asChild :
                ...
                tout en conservant le style et l'accessibilité, et
                nativeButton={false} précise que l'élément rendu n'est
                plus un vrai <button> (sinon warning console Base UI). */}
