 ...
 * appels HTTP du back-office vers le backend FastAPI. Toute préoccupation
 ...
 *
 * Copie de celui du portail B2B, à TROIS constantes près : l'URL de refresh
 * et la liste d'exclusion pointent vers l'espace plateforme. Le reste (mutex
 * de refresh, parsing par Content-Type, normalisation des erreurs) est
 * identique — et doit le rester.
 */
...
// 1. Importer getAdminRefreshTokenUrl() depuis le code généré créerait un
//    cycle d'imports (le généré importe déjà ce mutator) ;
// 2. Le cookie vetolib_admin_refresh est émis avec
//    Path=/api/v1/admin/auth/refresh : le navigateur ne le joint QUE sur
...
// /api/v1/admin/auth/me n'y figure PAS : son 401 doit déclencher le refresh
...
export const customFetch = async <T>(
  ...
  // le back-office (:3003) et l'API (:8000) sont des origines différentes.
  const doFetch = () =>
    fetch(`${BASE_URL}${url}`, {
      ...options,
      // Cookies JWT HttpOnly (vetolib_admin_access / vetolib_admin_refresh)
