/**
 * Composant Select (preset shadcn/ui, variante Base UI).
 *
 * Liste déroulante accessible construite sur la primitive Select de
 * Base UI. Deux points à connaître dans ce projet :
 * - composant CONTRÔLÉ (value/onValueChange, null = rien de choisi) :
 *   dans les formulaires react-hook-form, il passe par <Controller> ;
 * - la prop `items` du Root ([{value, label}]) sert à SelectValue pour
 *   afficher le LIBELLÉ de la valeur choisie (sinon la valeur brute,
 *   illisible quand c'est un UUID).
 */
"use client"
