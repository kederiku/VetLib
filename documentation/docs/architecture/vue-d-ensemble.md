...
description: "Les six briques de VetoLib et qui parle à qui."
...
d'architecture décrits dans cette section : deux portails clients distincts et une base
de données partagée mais compartimentée. S'y ajoute une troisième application, le
back-office par lequel l'équipe de la plateforme administre l'ensemble — d'où trois
espaces d'authentification cloisonnés, et non deux.
...
| `frontend-admin/` | Le back-office de la plateforme (`:3003`)        | Next.js App Router                    |
...
flowchart LR
  ...
  E -->|"fetch, credentials: include"| API
...
**aucune flèche entre un frontend et l'API côté serveur**. Les trois applications Next.js
sont servies au navigateur, et c'est le navigateur — donc le poste de l'utilisateur —
...
hooks TypeScript des trois applications. Ce n'est pas un appel à l'exécution mais une
...
scheduler** dans Docker. Les trois applications Next.js, elles, tournent **hors Docker**
(`make dev-b2c`, `make dev-b2b`, `make dev-admin`) : le rechargement à chaud de Next.js
est nettement plus rapide en natif que dans un conteneur avec un volume monté.
...
Les trois applications Next.js — et ce site de documentation — sont quatre projets npm
...
sont dupliqués à l'identique entre les trois applications. En échange :
...
- **Dependabot surveille les quatre répertoires npm séparément** — une entrée par projet
  dans `.github/dependabot.yml`, npm restant un seul écosystème — et ouvre donc des
  demandes de fusion ciblées au lieu d'un unique lot mélangeant les quatre ;
