...
async def create_pet(
    ...
    created = await use_case.execute(
        CreatePetCommand(owner_id=current.id, name=body.name, species=body.species)
    )
    return PetResponse.from_dto(created)


@patients_router.patch("/{pet_id}", operation_id="updatePet")
async def update_pet(
    ...
    updated = await use_case.execute(
        UpdatePetCommand(pet_id=pet_id, owner_id=current.id, name=body.name, species=body.species)
    )
